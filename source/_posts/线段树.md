...
template <typename intType> struct ZKWTr {
    int n;
    std::vector<u8> _log;
    std::vector<intType> tr, tag_mul;
    std::vector<size_t> sz;
    ZKWTr(size_t size) {
        n = size;
        _log.assign(n + 1, 0);
        for (int i = 2; i < n + 1; i++) _log[i] = _log[i / 2] + 1;
        tr.assign(2 * n + 2, 0);
        tag_mul.assign(2 * n + 2, 0);
        sz.assign(2 * n + 2, 1);
    }
    ...
    void push_down(int u) {
        if (tag_mul[u]) {
            modify(l_ch, tag_mul[u]);
            modify(r_ch, tag_mul[u]);
            tag_mul[u] = 0;
        }
    }
    intType query(int l, int r) {
        l += n, r += n + 1;
        for (int i = _log[n] + 1; i; i--) push_down(l >> i), push_down(r >> i);
        intType rst = 0;
        for (; l < r; l >>= 1, r >>= 1) {
            if (l & 1) rst += tr[l++];
            if (r & 1) rst += tr[--r];
